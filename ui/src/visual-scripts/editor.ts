...
import { backupScript, getRun, lifecycle, restoreScript, runManual, saveRevision, updateScript, validateGraph } from './api';
...
export class VisualScriptEditor extends HTMLElement {
  ...
  private selectedRun: VisualScriptRun | null = null;
  private busy = '';
  private notice = '';
  ...
  private render(): void {
    ...
    this.innerHTML = `
      <style>
        ...
        visual-script-editor .vse-bottom{display:grid;grid-template-columns:1fr 1fr;border-top:1px solid var(--border-color);min-height:0}.vse-bottom>section{overflow:auto;padding:8px 11px}.vse-bottom>section+section{border-left:1px solid var(--border-color)}.vse-problem,.vse-run,.vse-empty-message{font-size:13px;line-height:1.4;padding:5px 6px;border-radius:3px}.vse-problem,.vse-run{cursor:pointer}.vse-problem:hover,.vse-run:hover,.vse-run.selected{background:color-mix(in srgb,var(--content-text) 7%,transparent)}.vse-trace{margin-top:9px;padding-top:7px;border-top:1px solid var(--border-color)}.vse-trace-event{margin:5px 0;padding:6px;border:1px solid var(--border-color);border-radius:4px;font-size:12px}.vse-trace-event pre{margin:5px 0 0;padding:6px;overflow:auto;background:color-mix(in srgb,var(--content-text) 5%,transparent);border-radius:3px;font:12px/1.35 monospace;white-space:pre-wrap;word-break:break-word}.error{color:var(--status-bad-color,#f87171)}.warning{color:var(--status-warning-color,#fbbf24)}
      ...
      <div class="vse-bottom">
        ...
        <section><h3>Recent runs</h3>${this.runs.length ? this.runs.map(run => `<div class="vse-run ${this.selectedRun?.runId===run.runId?'selected':''}" data-run-id="${esc(run.runId)}"><b>${esc(run.status)}</b> · ${run.nodesExecuted} nodes · ${run.durationMs} ms<br><span style="opacity:.6">${esc(new Date(run.startedAt).toLocaleString())}</span></div>`).join('') : '<div class="vse-empty-message" style="opacity:.6">No runs yet.</div>'}${this.traceMarkup()}</section>
      </div>`;
  ...
  private traceMarkup(): string {
    ...
    const trace = this.selectedRun.trace ?? [];
    if (!trace.length) return `<div class="vse-trace"><h3>Run trace</h3><div class="vse-empty-message" style="opacity:.6">${this.selectedRun.status === 'queued' || this.selectedRun.status === 'running' ? 'Run in progress…' : 'No trace events recorded.'}</div></div>`;
    return `<div class="vse-trace"><h3>Run trace</h3>${trace.map(event => {
      ...
      return `<div class="vse-trace-event"><b>${esc(name)}</b> · ${esc(event.status)}${event.port?` · ${esc(event.port)}`:''}${event.message?`<br><span>${esc(event.message)}</span>`:''}<pre>${esc(payload)}</pre></div>`;
    }).join('')}</div>`;
  }
  ...
  private bind(): void {
    ...
    this.querySelectorAll<HTMLElement>('[data-run-id]').forEach(item=>item.addEventListener('click',()=>{const id=item.dataset.runId;if(id)void this.loadRun(id)}));
  ...
  private async triggerManual(triggerNodeId:string):Promise<void>{if(this.busy||this.script.desiredState!=='running'||!triggerNodeId)return;try{this.busy='trigger';this.notice='Queuing manual trigger…';this.render();const run=await runManual(this.script.id,triggerNodeId);this.runs=[run,...this.runs.filter(item=>item.runId!==run.runId)];this.selectedRun=run;this.busy='';this.notice='Manual trigger queued';this.render();void this.pollRun(run.runId)}catch(error){this.failed(error)}}
  private async loadRun(runId:string):Promise<void>{try{this.selectedRun=await getRun(this.script.id,runId);this.runs=this.runs.map(run=>run.runId===runId?this.selectedRun!:run);this.render()}catch(error){this.failed(error)}}
  private async pollRun(runId:string):Promise<void>{for(const delay of [100,200,400,800,1500,2500]){await new Promise(resolve=>setTimeout(resolve,delay));if(!this.isConnected)return;try{const run=await getRun(this.script.id,runId);this.runs=this.runs.map(item=>item.runId===runId?run:item);this.selectedRun=run;this.notice=run.status==='queued'||run.status==='running'?'Manual trigger running':`Manual run ${run.status}`;this.render();if(run.status!=='queued'&&run.status!=='running')return}catch{return}}}
