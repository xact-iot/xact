...
func coreDefinitions() []NodeDefinition {
	...
	field := ParameterDefinition{Name: "field", Label: "Field", Type: "string", Description: "Message field; leave blank or use value/$value for the main value"}
	...
	return []NodeDefinition{
		...
		{Type: "core.tag-changed", TypeVersion: 1, Name: "Tag Changed", Description: "Starts a run when a matching RTDB tag changes", Category: "Triggers", Icon: "⌁", Outputs: out, Parameters: []ParameterDefinition{{Name: "pathPattern", Label: "Tag path", Type: "tag-path", Required: true, Description: "Exact or wildcard path, for example SITE.*.Status.Running"}, {Name: "triggerOnStart", Label: "Trigger on start", Type: "boolean", Default: false, Description: "Read and output the current tag value when the script starts, if it is defined"}}, Available: true},
		...
		{Type: "core.set-tag", TypeVersion: 1, Name: "Set Tag", Description: "Writes a value to an RTDB tag", Category: "Actions", Icon: "⇱", Inputs: in, Outputs: out, Parameters: []ParameterDefinition{{Name: "tagPath", Label: "Tag path", Type: "tag-path", Required: true}, {Name: "source", Label: "Source", Type: "select", Options: []string{"value", "configured"}, Default: "value"}, {Name: "value", Label: "Configured value", Type: "json"}}, RequiredCaps: []string{"tag-write"}, Available: true, OutputNode: true},
		{Type: "core.send-control", TypeVersion: 1, Name: "Send Control", Description: "Sends a command through the device control path", Category: "Actions", Icon: "➤", Inputs: in, Outputs: out, Parameters: []ParameterDefinition{{Name: "deviceName", Label: "Device", Type: "string", Required: true}, {Name: "tagPath", Label: "Control path", Type: "tag-path", Required: true}, {Name: "source", Label: "Source", Type: "select", Options: []string{"value", "configured"}, Default: "value"}, {Name: "value", Label: "Configured value", Type: "json"}, number("timeout", "Timeout (seconds)", 10)}, RequiredCaps: []string{"device-control"}, Available: true, OutputNode: true},
...
func edgeTriggerParameters() []ParameterDefinition {
	return []ParameterDefinition{
		{Name: "pathPattern", Label: "Tag path", Type: "tag-path", Required: true, Description: "Exact or wildcard RTDB tag path"},
