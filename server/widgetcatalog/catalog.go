...
func BuiltIn() Catalog {
	return Catalog{
		...
		Widgets: []Widget{
			...
			systemWidget("mobile-app-config-widget", "Mobile App Configuration", "mobile", "Configures organisation-specific device-list parents and the default mobile dashboard.", 12, 14, 8, 8, []string{"mobile-app.read", "mobile-app.write"}, map[string]any{"organisationScope": "SystemAdmin can switch organisation; all other roles are fixed to their active organisation.", "deviceTabs": "Each configured device parent node becomes a tab in the mobile device list."}),
			...
			systemWidget("notifications-widget", "Notifications", "bell", "Configures notification profiles, role/user recipients, and email/Telegram/FCM channel settings.", 12, 24, 8, 12, []string{"notifications.view", "notifications.manage"}, map[string]any{"tabs": []string{"profiles", "channels"}, "channels": []string{"email", "telegram", "fcm"}}),
