...
func TestSQLiteEventsAndMetricQueries(t *testing.T) {
	...
	points := []sqldb.MetricEntry{
		...
		{DeviceName: "LA_LongBeach.AirQuality.AQ-S-0001", MetricName: "air.aqi", Timestamp: now.Add(3 * time.Second), Value: 42},
		{DeviceName: "LA_LongBeach.AirQuality.AQ-S-0140", MetricName: "air.aqi", Timestamp: now.Add(4 * time.Second), Value: 84},
	}
	...
	if err != nil || len(airQualityPath) != 1 || airQualityPath[0].Name != "air.aqi" || len(airQualityPath[0].Data) != 1 || airQualityPath[0].Data[0].Value != 42 {
